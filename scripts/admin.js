...
const clearOrdersModal = new bootstrap.Modal(document.getElementById("clearOrdersModal"))
...
function saveProduct() {
  ...
  if (currentProductId) {
    product.id = Number.parseInt(currentProductId)
...
function getProductNameById(productId) {
  // Load products from admin_products in localStorage
...
function displayOrders(filteredOrders = null) {
  ...
  ordersToDisplay.forEach((order, index) => {
    ...
    // Get product names from admin_products
  ...
  if (orderProducts && orderProducts.length > 0) {
    ...
    productsHtml = orderProducts
      .map((orderProduct) => {
        ...
        const productName =
          fullProduct?.name || orderProduct.name || orderProduct.title || orderProduct.productName || "Unknown Product"
        const productPrice = fullProduct?.price || orderProduct.price || orderProduct.cost || "N/A"
        const productImage =
          fullProduct?.frontImage || orderProduct.frontImage || orderProduct.image || orderProduct.img
        ...
        return `
          <div class="card mb-2">
            <div class="card-body">
              <div class="row">
                <div class="col-md-3">
                  ${imageSrc ? `<img src="${imageSrc}" alt="${productName}" class="img-fluid rounded" style="max-height: 150px; object-fit: cover;">` : '<div class="bg-light rounded d-flex align-items-center justify-content-center" style="height: 150px;"><span class="text-muted">No Image</span></div>'}
                </div>
                <div class="col-md-9">
                  ...
                  ${fullProduct ? `<p class="card-text"><small class="text-success">✓ Product details loaded from admin_products</small></p>` : `<p class="card-text"><small class="text-warning">⚠ Using order data (product not found in admin_products)</small></p>`}
                </div>
              </div>
            </div>
          </div>
